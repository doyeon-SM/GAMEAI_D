...
public class GameManager : MonoBehaviour
{
    ...
    IEnumerator GameLoop()
    {
        ...
        {
            LogMessage($"Trun {currentTurn + 1} starts");
            while (aiPlayers.FindAll(player => player.IsAlive).Count > 1)
            {
                LogMessage($"Action {actionturns + 1} starts");
                ...
                foreach (string deadPlayerName in AIPlayer.deathList)
                {
                    ...
                    if (deadPlayer != null)
                    {
                        ...
                        LogMessage($"{deadPlayer.playerName} has been ranked {rank}");
                    }
                }
                ...
                if (aiPlayers.FindAll(player => player.IsAlive).Count <= 1)
                {
                    if (aiPlayers.FindAll(player => player.IsAlive).Count == 1)
                    {
                        LogMessage("Game Over. Winner: " + aiPlayers.Find(player => player.IsAlive)?.playerName);

                        ranklist.Add($"{aiPlayers.Find(player => player.IsAlive)?.playerName}: {8}위");
                    }
                    else
                    {
                        LogMessage("Game Over. No clear winner.");
                    }
                    LogMessage("Final Rank List: " + string.Join(" ", ranklist));
        ...
        LogMessage("All turns completed.");
        // reset
        currentTurn = 0;
        actionturns = 0;
        rank = 1;
    ...
    IEnumerator ExtractGenesFromRank()
    {
        ...
        for (int i = 0; i < aiPlayers.Count; i++)
        {
            ...
            LogMessage($"{player.playerName}: genes: " + string.Join(", ", player.genes));
        ...
        foreach (var (parent1, parent2) in pairs)
        {
            ...
            LogMessage($"Crossover between {parent1.playerName} and {parent2.playerName} at point {crossoverPoint}");
            LogMessage($"{parent1.playerName} genes after crossover: " + string.Join(", ", parent1.genes));
            LogMessage($"{parent2.playerName} genes after crossover: " + string.Join(", ", parent2.genes));
        ...
        foreach (AIPlayer player in aiPlayers)
        {
            ...
            for (int i = 0; i < player.genes.Length; i++)
            {
                if (Random.value < 0.05f) // 5% 확률로 돌연변이 발생
                {
                    ...
                    LogMessage($"Mutation {player.playerName} index {i}: {oldGene} -> {newGene}");
    ...
    private static string logFilePath = "game_log.txt"; //로그 저장위치   를 logFilePath에 저장한다?

    void LogMessage(string message) // 이걸 Debug.log 대신 작동시는 구조
    {
        // 파일에 로그 작성
        using (StreamWriter writer = new StreamWriter(logFilePath, true)) // true: 기존 내용에 추가
        {
